...
7. Adaptive scheduling uses an exact 0/1 knapsack over actions that can earn utility in
   the current contact: whole fault packets and final raw completion chunks. It then uses
   otherwise stranded capacity for zero-utility raw fragments selected by full-completion
   value density. This gives adaptive a realistic way to work toward future full raw
   completions without awarding partial raw value.
8. Adaptive stale suppression removes older regular faults when a newer fault state for
...
Run:
    python downlink2_fragmented.py
...
def packet_utility(
    ...
    if packet.packet_class == EMERGENCY:
        ...
        return cfg.emergency_class_value * fault_content_multiplier(packet) * latency_decay * stale_factor
    ...
    if packet.packet_class == REGULAR:
        ...
        return cfg.regular_class_value * fault_content_multiplier(packet) * latency_decay * stale_factor
...
def adaptive_raw_fragment_score(
    ...
    progress_bonus = 1.0 + 0.50 * raw_progress_fraction(packet)
    return progress_bonus * completion_value / max(1, remaining)
...
def adaptive_select(
    ...
    """Adaptive per-contact scheduling with zero-utility raw fragmentation."""
    ...
    for packet in queue:
            ...
            if remaining_raw <= capacity_kb:
                ...
                candidates.append(
                    Transmission(
                        ...
                        selection_reason="adaptive_knapsack_raw_completion",
                        planning_score=completion_value / max(1, remaining_raw),
                    )
                )
        ...
        if packet.size_kb <= capacity_kb:
            value = packet_utility(packet, contact_time_h, cfg, latest_fault_time_by_parcel)
            candidates.append(
                Transmission(
                    packet=packet,
                    transmit_kb=int(packet.size_kb),
                    completes_packet=True,
                    is_final_raw_chunk=False,
                    selection_reason="adaptive_knapsack_fault",
                    planning_score=value / max(1, int(packet.size_kb)),
                )
            )
    ...
    raw_candidates.sort(
        key=lambda p: (
            -adaptive_raw_fragment_score(p, contact_time_h, cfg, latest_fault_time_by_parcel),
            p.created_at_h,
            p.packet_id,
        )
    )
    ...
    for packet in raw_candidates:
        ...
        selected.append(
            Transmission(
                ...
                completes_packet=completes,
                is_final_raw_chunk=completes,
                selection_reason="adaptive_fragment_completes_raw" if completes else "adaptive_zero_utility_raw_fragment",
                planning_score=adaptive_raw_fragment_score(packet, contact_time_h, cfg, latest_fault_time_by_parcel),
            )
        )
        remaining_capacity -= tx_kb
