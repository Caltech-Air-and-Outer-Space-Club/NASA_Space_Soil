...
7. Raw-data requests carry a ground-assigned value class. The class affects full
   completion utility and deadline pressure, but partial fragments still receive zero
   decision utility.
8. Adaptive scheduling uses an exact 0/1 knapsack over utility-bearing actions in
...
9. Operational decision deadlines are separate from the 14-day physical storage limit.
...
10. Adaptive stale suppression removes older regular faults when a newer fault state for
   the same parcel is known onboard. Adaptive also uses ground-mediated constellation
   duplicate suppression: once any satellite has downlinked a newer parcel fault state,
   older regular faults for that parcel on other satellites can be suppressed. Emergency
   fault packets are retained as alert/audit products; their utility is still penalized
   when a newer state exists.
...
RAW_REQUEST_CLASS_PROBS = (0.14, 0.20, 0.18, 0.16, 0.12, 0.20)

RAW_REQUEST_CLASS_MULTIPLIERS = {
    "emergency_followup": 1.65,
    "model_uncertainty_audit": 1.25,
    "customer_requested_review": 1.35,
    "high_value_crop": 1.45,
    "calibration_validation": 1.05,
    "random_quality_check": 0.70,
}

RAW_REQUEST_CLASS_DEADLINE_MULTIPLIERS = {
    "emergency_followup": 0.55,
    "model_uncertainty_audit": 0.85,
    "customer_requested_review": 0.75,
    "high_value_crop": 0.80,
    "calibration_validation": 1.20,
    "random_quality_check": 1.30,
}
...
class SimConfig:
    ...
    # Ground-mediated constellation duplicate suppression. A regular fault is
    # redundant if ground has already received a newer fault state for the same parcel.
    enable_constellation_duplicate_suppression: bool = True
...
class Packet:
    ...
    raw_request_class: Optional[str] = None
...
class SchedulerResult:
    ...
    stale_removed: int
    constellation_duplicate_removed: int
    expired_removed: int
...
def generate_packets(rng: np.random.Generator, parcels: pd.DataFrame, cfg: SimConfig) -> List[Packet]:
    ...
    for sat_id in range(cfg.n_satellites):
        ...
        for t_h in request_times:
            ...
            raw_request_class = str(rng.choice(RAW_REQUEST_CLASSES, p=RAW_REQUEST_CLASS_PROBS))
            ...
            packets.append(
                Packet(
                    ...
                    raw_request_class=raw_request_class,
                    image_pixels_x=px,
                    image_pixels_y=py,
                    image_bands=bands,
...
def raw_request_class_multiplier(packet: Packet) -> float:
    """Ground-assigned raw-request priority multiplier."""
    if packet.packet_class != RAW:
        return 1.0
    return float(RAW_REQUEST_CLASS_MULTIPLIERS.get(packet.raw_request_class or "random_quality_check", 1.0))


def raw_request_deadline_multiplier(packet: Packet) -> float:
    """Ground-assigned deadline pressure for raw-data review tasks."""
    if packet.packet_class != RAW:
        return 1.0
    return float(RAW_REQUEST_CLASS_DEADLINE_MULTIPLIERS.get(packet.raw_request_class or "random_quality_check", 1.0))
...
def decision_deadline_h(packet: Packet, cfg: SimConfig) -> float:
    ...
    if packet.packet_class == RAW:
        return cfg.raw_deadline_h * raw_request_deadline_multiplier(packet)
...
def packet_utility(
    ...
    if packet.packet_class == RAW:
        ...
        return (
            cfg.raw_full_completion_value
            * raw_request_class_multiplier(packet)
            * raw_content_multiplier(packet)
            * latency_decay
            * operational_deadline_factor
        )
...
def suppress_constellation_duplicates(
    queue: Sequence[Packet],
    latest_ground_fault_time_by_parcel: Dict[int, float],
) -> Tuple[List[Packet], int]:
    """Remove regular faults made redundant by newer ground-known parcel states.

    This is deliberately ground-mediated: a satellite only benefits after some
    satellite has already downlinked a newer fault state for the same parcel. It
    does not use future observations or hidden global truth.
    """
    kept: List[Packet] = []
    removed = 0
    ...
    for packet in queue:
        if packet.packet_class == REGULAR:
            if latest_ground_fault_time_by_parcel.get(packet.parcel_id, -math.inf) > packet.created_at_h:
                removed += 1
                continue
        kept.append(packet)
...
def merge_latest_fault_times(*sources: Dict[int, float]) -> Dict[int, float]:
    """Combine latest-fault-time maps by parcel."""
    merged: Dict[int, float] = {}
    for source in sources:
        for parcel_id, t_h in source.items():
            if t_h > merged.get(parcel_id, -math.inf):
                merged[parcel_id] = t_h
    return merged
...
def run_scheduler(
    ...
    deliveries: List[Dict] = []
    stale_removed = 0
    constellation_duplicate_removed = 0
    expired_removed = 0
    next_packet_idx = 0
    ...
    for contact_index, contact in enumerate(contacts_sorted):
        ...
        effective_latest_fault_time = merge_latest_fault_times(
            latest_fault_time[contact.sat_id],
            latest_ground_fault_time_by_parcel,
        )

        if scheduler == "adaptive":
            sat_queue, removed_now = suppress_stale_faults(sat_queue, latest_fault_time[contact.sat_id])
            stale_removed += removed_now
            if cfg.enable_constellation_duplicate_suppression:
                sat_queue, removed_duplicates_now = suppress_constellation_duplicates(
                    sat_queue,
                    latest_ground_fault_time_by_parcel,
                )
                constellation_duplicate_removed += removed_duplicates_now
        ...
        for tx in transmissions:
            ...
            if packet.packet_class == RAW:
                ...
                utility = packet_utility(packet, contact.start_h, cfg, effective_latest_fault_time) if completes_now else 0.0
                ...
                utility = packet_utility(packet, contact.start_h, cfg, effective_latest_fault_time)
            ...
            deliveries.append(
                {
                    ...
                    "raw_request_class": packet.raw_request_class,
            ...
            if packet_completed and is_fault(packet):
                prev_ground_t = latest_ground_fault_time_by_parcel.get(packet.parcel_id, -math.inf)
                if packet.created_at_h > prev_ground_t:
                    latest_ground_fault_time_by_parcel[packet.parcel_id] = packet.created_at_h
    ...
    return SchedulerResult(
        ...
        constellation_duplicate_removed=constellation_duplicate_removed,
...
def summarize_result(result: SchedulerResult, packets: Sequence[Packet]) -> Dict[str, float]:
    ...
    return {
        ...
        "constellation_duplicate_removed": float(result.constellation_duplicate_removed),
...
def raw_value_class_breakdown(results: Dict[str, SchedulerResult]) -> pd.DataFrame:
    """Completed raw products by ground-request value class."""
    rows = []
    for scheduler, result in results.items():
        df = delivery_dataframe(result)
        if df.empty:
            continue
        completed_raw = df[(df["packet_class"] == RAW) & (df["product_class"] == RAW_FULL)].copy()
        if completed_raw.empty:
            continue
        grouped = completed_raw.groupby("raw_request_class", dropna=False, as_index=False).agg(
            completed_full_raw_requests=("raw_request_id", "nunique"),
            completed_raw_kb=("packet_size_kb", "sum"),
            delivered_utility=("utility", "sum"),
...
def monte_carlo_medians(seeds: Iterable[int], cfg: SimConfig) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    scheduler_medians = all_summary.groupby("scheduler", as_index=False).agg(
        ...
        median_stale_superseded_removed=("stale_superseded_removed", "median"),
        median_constellation_duplicate_removed=("constellation_duplicate_removed", "median"),
    )
...
def format_summary_table(summary: pd.DataFrame) -> pd.DataFrame:
    cols = [
        ...
        "constellation_duplicate_removed",
    ]
...
def print_report(single_seed: int = 7, mc_seed_count: int = 30) -> None:
    ...
    raw_value_counts = pd.Series([p.raw_request_class for p in packets if p.packet_class == RAW]).value_counts().sort_index()
    print("Generated raw request value classes: " + ", ".join(f"{k}={v}" for k, v in raw_value_counts.items()))
    ...
    print("Completed full raw requests by ground value class")
    print(raw_value_class_breakdown(results).to_string(index=False, float_format=lambda x: f"{x:,.3f}"))
    print()
